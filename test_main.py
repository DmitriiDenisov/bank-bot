...
def test_get_trans():
    ...
    assert type(json.loads(response.data.decode('utf-8'))['results']) == type(list())
...
def test_own_transfer():
    response = client.post('/own_transfer',
                        query_string={'curr_from': 'usd', 'curr_to': 'aed', 'amount': 1},
                        # content_type='multipart/form-data',
                        headers={
                            'key': TEST_TOKEN})
    assert response.status_code == 200
    ...
    response = client.post('/own_transfer',
                        query_string={'curr_from': 'aed', 'curr_to': 'usd', 'amount': 3.67},
                        # content_type='multipart/form-data',
                        headers={
                            'key': TEST_TOKEN})
    assert response.status_code == 200
...
def test_topup_and_transaction():
    response = client.post('/topup',
                           ...
                           headers={
                               'key': TEST_TOKEN})
    assert response.status_code == 200
    assert json.loads(response.data.decode('utf-8')) == {"message": "TopUp successful!"}
    ...
    response = client.post('/do_transaction',
                        query_string={'customer_id_to': 33, 'amount': 10, 'currency': 'aed'},
                        # content_type='multipart/form-data',
                        headers={
                            'key': TEST_TOKEN})
    assert response.status_code == 200
    assert json.loads(response.data.decode('utf-8')) == {"message": "Transaction made!"}


def test_main_new():
    response = client.post('/',
                           content_type='multipart/form-data',
                           data={'method': 'auth', 'email': 'dmitryhse@gmail.com', 'password': 'it_is_fake_password'},
                           headers={
                               'key': TEST_TOKEN}
                           )
    # resp['method'] == 'auth'
...
def test_main():
    response = client.post('/',
                        content_type='multipart/form-data',
                        data={'method': 'sign-up', 'email': 'dmitryhse@gmail.com', 'password': 'fake', 'confirm': 'fake'},
                        headers={
                            'key': TEST_TOKEN}
                        )
